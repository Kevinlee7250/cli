...
• 독자에게 말 걸기: "이 부분 궁금하셨죠?", "저만 헷갈렸던 건 아니겠죠?" 1~2회
...
• H2 제목: 사람이 실제로 검색하거나 말할 법한 표현으로
  ✗ "✅ 핵심 방법 TOP 5" → ✓ "처음 시작할 때 제일 헷갈리는 것"
  ✗ "주의사항 및 고려사항" → ✓ "이건 자료 찾아보고 나서야 알았어요"
• 섹션마다 다른 방식으로 쓰기: 어떤 건 사례 분석, 어떤 건 숫자 비교, 어떤 건 Q&A
...
• "이런 이야기는 처음 봤다"고 느낄 각도: "실패 사례를 모아 보면", "안내문과 실제가 달랐던 지점", "{y}년부터 달라진 점"

━━━ Google AdSense 정책 준수 (가치 없는 콘텐츠 위반 방지) ━━━
① 원본성 — 독창적 관점·인사이트 필수. 인터넷에 흔한 정보 나열 금지. 반드시 직접 조사해 확인한 구체적 수치·출처·비교 포함. ⚠️ 하지 않은 경험을 지어내는 것은 원본성이 아니라 AdSense 거절 사유입니다
② SEO — 키워드 10~14회 자연 분산 — 숫자를 맞추려고 억지로 넣지 말 것. 어색하면 적게 쓰는 편이 낫습니다, 제목·내용 일치, 낚시성 제목 금지
...
각 H2 섹션마다 최소 3개 단락, 구체적 사례·수치·출처 반드시 포함
...
• "저는", "제가", "솔직히" 등 의견·판단형 1인칭을 자연스럽게 사용 (하지 않은 경험을 지어내는 표현은 금지)
• 도입부: 독자가 이 주제를 찾게 되는 상황, 또는 이번 편에서 확인한 자료의 핵심으로 시작
• 확신이 없는 부분은 솔직하게: "이게 맞는 건지 저도 100%는 모르겠지만요"
• 독자에게 말 걸기: "이 부분 궁금하셨죠?", "찾아보기 전엔 저도 헷갈렸어요" 1~2회
...
• 섹션마다 다른 방식: 어떤 건 사례 분석, 어떤 건 숫자 비교, 어떤 건 Q&A
• 목록은 진짜 나열이 필요할 때만
• 유형별 구조 가이드:
{structure_guide}

━━━ Google AdSense 2025 정책 준수 (가치 없는 콘텐츠 위반 방지) ━━━
① 원본성·품질 — 독창적 관점·인사이트 필수. 인터넷에 흔한 정보 나열 금지. 직접 조사해 확인한 구체적 수치·출처·비교 포함. ⚠️ 하지 않은 경험을 지어내는 것은 원본성이 아니라 AdSense 거절 사유입니다
...
━━━ 드라마 회차 정리 구조 (정보형 — 반드시 이 요소들을 모두 포함) ━━━
이 글은 이미 방영된 드라마 {drama_ep_num}화의 회차 정리입니다.{binfo_line}
⚠️ 이 글의 화자는 시청자가 아니라 '자료를 모아 정리하는 사람'입니다.
   "봤는데 / 보는 내내 / 시청했더니 / 소름 돋았다" 같은 시청 경험 표현 금지.
   대신 "기사에 따르면", "시청자 반응을 모아 보면", "자료를 보면".
• 도입: 이번 화가 화제가 된 이유 한 줄 + 스포일러 수위 안내
• H2: 편성·시청률 — 방송사·편성·부작과 회차별 시청률 추이를 숫자로 (400자 이상)
  조사기관명을 밝힐 것 (예: "닐슨코리아 기준 전국 유료가구").
  자료팩에 수치가 없으면 이 H2는 통째로 생략 — 추정치를 만들어내지 말 것
• H2: {drama_ep_num}화 줄거리 핵심 정리 (600자 이상 — 장면 순서대로 구체적으로)
• H2: 화제 장면 TOP 3 — 기사·반응에서 실제로 많이 언급된 순서로 (600자 이상)
• H2: 대사와 연기에 대한 평가 — 매체·시청자가 어떻게 평가했는지 인용·요약 (400자 이상)
  "연기가 좋았다"는 글쓴이 감상이 아니라 "○○ 매체는 ~라고 평했다"로 서술
• H2: 시청자 반응·화제성 — 커뮤니티·기사 반응 정리 (400자 이상).
  화제성 지표가 자료팩에 있으면 조사기관과 주차를 함께 밝힐 것
  (예: "굿데이터 펀덱스 9월 2주차 드라마 부문 1위, 점유율 18.46%")
• H2: 다음 화 관전 포인트 — 자료팩에서 확인되는 떡밥·복선만 정리 (300자 이상)
• 라벨(labels)은 드라마·리뷰 관련으로만: ["드라마리뷰","{keyword}","드라마"] 등 (금융·경제 등 무관 라벨 금지)

⚠️⚠️ 절대 규칙 — 예상 시나리오·시청 경험 주장 금지:
• 이 글은 방송을 본 사람이 아니라 자료를 정리한 사람이 씁니다 —
  1인칭 시청 표현("봤는데", "보는 내내", "직접 보니")을 쓰지 마세요
• 시청률·화제성 수치는 자료팩에 있는 것만, 반드시 조사기관명과 기준을 함께
