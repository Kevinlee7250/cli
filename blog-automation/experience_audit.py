...
HIGH_PATTERNS = [
    ...
    # 2026-09-20 좁힘 — 9/20 발행된 "라운딩 비용을 항목별로 정리해봤습니다"가
    # high로 잡혔습니다. '정리해봤'의 '해봤'을 본 것인데, 이건 글을 쓴 행위이지
    # 겪은 일이 아닙니다. 오히려 우리가 쓰라고 지시한 조사·정리형 문장입니다.
    # high는 본문 리라이팅·비공개 판정을 부르는 등급이라 그냥 둘 수 없습니다.
    # 조사 동사(정리·알아·찾아·살펴…) 뒤의 '해봤'은 제외합니다.
    (re.compile(
        r"(?<!정리)(?<!알아)(?<!찾아)(?<!살펴)(?<!비교)(?<!조사)(?<!계산)"
        r"(?<!검색)(?<!따져)(?<!짚어)(?<!뜯어)(?<!훑어)(?<!점검)"
