...
def _google_trends_realtime(count: int = 20) -> list[dict]:
    ...
    try:
        ...
        if r.status_code != 200:
            logger.warning(f"Google Trends RSS 오류: HTTP {r.status_code}")
            return []
        ...
        for i, item in enumerate(root.iter("item")):
            ...
            if traffic_el is not None:
                traffic = re.sub(r"<[^>]+>", "", traffic_el.text or "").strip()
            results.append({
                "keyword": kw,
                "rank": i + 1,
                "traffic": traffic,
        ...
        logger.info(f"구글 트렌드: {len(results)}개 수집")
    except Exception as e:
        logger.warning(f"Google Trends RSS 수집 실패: {e}")
    return results


_NAVER_RSS_CATEGORIES = {
    "경제": "https://www.rss.naver.com/main/rss.nhn?code=economic",
    "사회": "https://www.rss.naver.com/main/rss.nhn?code=social",
    "IT과학": "https://www.rss.naver.com/main/rss.nhn?code=science",
    "생활문화": "https://www.rss.naver.com/main/rss.nhn?code=culture",
    "세계": "https://www.rss.naver.com/main/rss.nhn?code=world",
}
...
def _load_ai_recommended() -> list[dict]:
    ...
    try:
        ...
        return [{"keyword": kw, "source": "ai_recommended"} for kw in kws if kw]
    except Exception as e:
